...
// ==========================================
// DATA ULAR DAN TANGGA (POSISI REVISI TERBARU)
// ==========================================
const ladders = {
  2: 38,
  7: 14,
...
function playSound(type) {
  ...
  try {
    if (type === "roll") {
      // Suara Kocokan Dadu (Beberapa pergeseran frekuensi acak)
      for (let i = 0; i < 4; i++) {
      ...
      // Suara Bidak Berjalan (Pop Ceria)
      const osc = audioCtx.createOscillator();
      const gain = audioCtx.createGain();
      osc.type = "sine";
      ...
      // Suara Naik Tangga (Arpeggio Nada Meluncur Naik)
      ...
      // Suara Turun Ular (Efek Peluit Meluncur Turun)
      const osc = audioCtx.createOscillator();
      const gain = audioCtx.createGain();
      osc.type = "sawtooth";
      ...
      // Suara Jawaban Salah (Nada Rendah / Buzz)
      const osc = audioCtx.createOscillator();
      const gain = audioCtx.createGain();
      osc.type = "sawtooth";
...
window.onload = function () {
  ...
  // Aktifkan Audio Context saat interaksi pertama pemain
...
// ==========================================
// STORAGE PERSISTENCE (LOCAL STORAGE)
// ==========================================
...
function loadLocalQuestions() {
  const savedQ = localStorage.getItem("snake_questions");
  questionBank = savedQ ? JSON.parse(savedQ) : [...defaultQuestions];
}

function saveLocalQuestions() {
  localStorage.setItem("snake_questions", JSON.stringify(questionBank));
  renderCrudList();
}
...
function getTileCoordinates(tileNumber) {
  if (tileNumber <= 0) return { x: -8, y: 92 }; // Posisi Luar Papan
...
function handleRollDice() {
  ...
  const interval = setInterval(() => {
    playSound("roll"); // Suara Kocokan Dadu Setiap Acakan
...
function showQuestion() {
  ...
  // BERSIHKAN KOTAK PENJELASAN UNTUK SOAL BARU
...
function handleAnswer(selectedIndex, qData) {
  ...
  setTimeout(() => {
    ...
    // RESET & HAPUS ISI KOTAK PENJELASAN
    feedback.innerText = "";
    feedback.className = "feedback-box hidden";
...
function movePlayer(playerIdx, steps) {
  ...
  const walkInterval = setInterval(() => {
    if (current < target) {
      current++;
      p.pos = current;
      playSound("step"); // Suara Langkah Bidak
      renderTokens();
    } else {
...
function checkSpecialTiles(player) {
  ...
  if (ladders[pos]) {
    playSound("ladder"); // Suara Naik Tangga Meriah
    ...
    playSound("snake"); // Suara Turun Ular Peluit Meluncur
